                    ...
                    <tbody>
                        <?php foreach ($rows as $row) : ?>
                            <tr>
                                ...
                                <td>
                                    <!-- <a href="<?= base_url('pengawas/download_laporan_barang_keluar/') . $row->id_laporan ?>" class="badge badge-primary"><i class="fas fa-arrow-alt-circle-down"></i> unduh</a> -->
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <a href="<?= base_url('pengawas/barang_keluar_success/') . $row->id_laporan; ?>" class="btn btn-success btn-sm">Ubah</a>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <input type="hidden" name="id_laporan" id="" value="<?= $row->id_laporan; ?>">
                    <button type="button" class="btn btn-secondary btn-sm" data-dismiss="modal">Batal</button>
                    <a href="<?= base_url('pengawas/barang_keluar_proses/') . $row->id_laporan; ?>" class="btn btn-success btn-sm">Ubah</a>
                </div>
            </div>
        </div>
    </div>
...
?>
    <div class="modal fade" id="feedback<?= $row->id_laporan; ?>" tabindex="-1" aria-labelledby="editKategoriLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="editKategoriLabel">Form Komentar Laporan Barang Keluar</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form action="<?= base_url('pengawas/feedback_barang_keluar/') . $row->id_laporan; ?><?= base_url('pengawas/feedback_barang_masuk/') . $row->id_laporan ?>" method="post" enctype="multipart/form-data">
                        <div class="mb-2">
                            <input type="hidden" name="id_laporan" value="<?= $row->id_laporan; ?>">
                            <label for="feedback">Komentar Laporan Barang Keluar</label>
    ...
    <div class="modal fade" id="unduh<?= $row->id_laporan; ?>" tabindex="-1" aria-labelledby="newSatuanModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    Pilih "Unduh" di bawah jika Anda akan mengunduh data laporan barang keluar dari tanggal <b><?= $tglAwal; ?></b> s.d. <b><?= $tglAkhir; ?></b>.
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary btn-sm" data-dismiss="modal">Batal</button>
                    <a href="<?= base_url('pengawas/download_laporan_barang_keluar/') . $row->id_laporan ?>" class="btn btn-success btn-sm">Unduh</a>
                </div>
            </div>
        </div>
    </div>
